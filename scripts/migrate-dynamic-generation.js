...
// (spec: 2026-09-24-dynamic-course-structure-fase5-generation-design, Task 1
// de Fase 5A). No se infiere nada de DB_HOST/DB_NAME — el operador tiene que
...
function assertExplicitStagingIntent() {
  if (process.env.MIGRATION_ENV !== 'staging') {
    console.error(
      ...
      '   staging, usá: MIGRATION_ENV=staging node scripts/migrate-dynamic-generation.js'
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  try {
    const sql = fs.readFileSync(
      path.resolve(__dirname, '../supabase-migration-dynamic-generation.sql'),
      'utf8',
    );
    await client.query('begin');
    await client.query(sql);
    await client.query('commit');
    console.log('✅ Migración dynamic-generation aplicada (o ya estaba aplicada — es idempotente).');
  } catch (err) {
