...
// (spec: 2026-09-24-dynamic-course-structure-fase4-manifest). No se infiere
// nada de DB_HOST/DB_NAME (los hosts reales de Supabase nunca contienen
// "staging" y DB_NAME siempre es "postgres") — el operador tiene que
// declararlo. Mismo patrón que migrate-course-blueprints.js.
function assertExplicitStagingIntent() {
  if (process.env.MIGRATION_ENV !== 'staging') {
    console.error(
      ...
      '   staging, usá: MIGRATION_ENV=staging node scripts/migrate-generation-manifests.js'
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  try {
    const sql = fs.readFileSync(
      path.resolve(__dirname, '../supabase-migration-generation-manifests.sql'),
      'utf8',
    );
    await client.query('begin');
    await client.query(sql);
    await client.query('commit');
    console.log('✅ Migración generation-manifests aplicada (o ya estaba aplicada — es idempotente).');
  } catch (err) {
