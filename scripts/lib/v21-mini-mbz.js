...
// sección 1 con UNA actividad h5pactivity de video interactivo:
...
/**
 * @param {{ courseTitle: string, activityName: string, packageFilename: string, h5p: Buffer, introHtml: string, mid: number }} p
 * @returns {Promise<Buffer>}
 */
async function buildMiniVideoMbz(p) {
  const { c, h } = load();
  ...
  // ── files ──
  const entries = h.videoActivityFileEntries({ packageFilename: p.packageFilename, h5p: p.h5p });
  put(entries[0].blobPath, p.h5p); // un solo blob para las dos entradas
  const fileIds = [1, 2];
...
<activity id="${aid}" moduleid="${mid}" modulename="h5pactivity" contextid="${ctx}">
  <h5pactivity id="${aid}">
    <name>${c.xmlEsc(p.activityName)}</name>
    <timecreated>${TS}</timecreated>
    <timemodified>${TS}</timemodified>
    <intro>${c.xmlEsc(p.introHtml)}</intro>
  ...
  put(
    ...
    c
      ...
      .replace('<showdescription>0</showdescription>', `<showdescription>${S.showdescription}</showdescription>`),
  );
  put(
    `${dir}/inforef.xml`,
  ...
  const sections = [
    { num: 0, name: '', summary: '' },
    { num: 1, name: 'Capítulo 1 — Video interactivo', summary: '' },
  ];
  for (const s of sections) {
    put(`sections/section_${s.num}/section.xml`, c.sectionXml(s, s.num === secnum ? String(mid) : '', TS));
  ...
  <contents>
    <activities>
      <activity>
        ...
        <title>${c.esc(p.activityName)}</title>
...
module.exports = { buildMiniVideoMbz, MINI_MBZ_TS: TS };
