...
// Cursia V2.1 — R3 (Blueprint v2 + perfiles de curso). Spec
// docs/v21/2026-09-26-cursia-v21-experience-audit.md §M/§S. Aditiva,
// idempotente y SOLO staging: la cablea deploy-staging.yml (nunca deploy.yml),
// después de migrate-course-blueprints.js (necesita courses/course_chapters).
//
...
function assertExplicitStagingIntent() {
  if (process.env.MIGRATION_ENV !== 'staging') {
    console.error(
      ...
      '   staging, usá: MIGRATION_ENV=staging node scripts/migrate-v21-blueprint-profiles.js'
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  try {
    const sql = fs.readFileSync(
      path.resolve(__dirname, '../supabase-migration-v21-blueprint-profiles.sql'),
      'utf8',
    );
    await client.query('begin');
    await client.query(sql);
    await client.query('commit');
    console.log('✅ Migración V2.1 R3 (activity_enabled, final_exam_enabled, activity_engine, course_profiles) aplicada (o ya estaba aplicada — es idempotente).');
  } catch (err) {
