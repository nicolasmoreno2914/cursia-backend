...
// Cursia V2.1 — R4 (Generation Manifest rulesVersion 3). Spec
// docs/v21/2026-09-26-cursia-v21-experience-audit.md §N. Aditiva,
// idempotente y SOLO staging: la cablea deploy-staging.yml (nunca deploy.yml),
// DESPUÉS de migrate-dynamic-generation-v2.js y migrate-invalidation.js
// (extiende gir_type_check/gir_type_scope/cgm_counts_consistent de v2, y
// reemplaza la función de scope que v2 re-crea en cada deploy).
//
...
function assertExplicitStagingIntent() {
  if (process.env.MIGRATION_ENV !== 'staging') {
    console.error(
      ...
      '   staging, usá: MIGRATION_ENV=staging node scripts/migrate-v21-manifest-v3.js'
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  try {
    const sql = fs.readFileSync(
      path.resolve(__dirname, '../supabase-migration-v21-manifest-v3.sql'),
      'utf8',
    );
    await client.query('begin');
    await client.query(sql);
    await client.query('commit');
    console.log('✅ Migración V2.1 R4 (tipos de item v3 en generation_item_runs + conteos v3 en course_generation_manifests) aplicada (o ya estaba aplicada — es idempotente).');
  } catch (err) {
