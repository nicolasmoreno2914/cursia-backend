...
// (spec: 2026-09-24-dynamic-course-structure-fase3-blueprint). No se infiere
...
// declararlo. Mismo patrón que migrate-dynamic-course-structure.js.
function assertExplicitStagingIntent() {
  if (process.env.MIGRATION_ENV !== 'staging') {
    console.error(
      ...
      '   staging, usá: MIGRATION_ENV=staging node scripts/migrate-course-blueprints.js'
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  try {
    const sql = fs.readFileSync(
      path.resolve(__dirname, '../supabase-migration-course-blueprints.sql'),
      'utf8',
    );
    await client.query('begin');
    await client.query(sql);
    await client.query('commit');
    console.log('✅ Migración course-blueprints aplicada (o ya estaba aplicada — es idempotente).');
  } catch (err) {
