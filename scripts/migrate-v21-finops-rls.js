...
// V2.1 RF-b (fix C2) — RLS + REVOKE de anon/authenticated sobre las 6 tablas
// FinOps y course_profiles (supabase-migration-v21-finops-rls.sql). Aditiva,
// idempotente y SOLO staging. Corre DESPUÉS de migrate-v21-finops.js (y de la
// migración R3 si course_profiles existe; si no, se omite esa tabla).
//
// Exporta applyV21FinopsRls(client) para los checks locales (Postgres
// desechable); el guardarraíl solo corre cuando se ejecuta como script.
//
function assertExplicitStagingIntent() {
  if (process.env.MIGRATION_ENV !== 'staging') {
    console.error(
      ...
      '   staging, usá: MIGRATION_ENV=staging node scripts/migrate-v21-finops-rls.js'
    );
    process.exit(1);
  }
}
...
const SQL_PATH = path.resolve(__dirname, '../supabase-migration-v21-finops-rls.sql');

async function applyV21FinopsRls(client, opts = {}) {
  await client.query(fs.readFileSync(opts.sqlPath || SQL_PATH, 'utf8'));
}
...
async function main() {
  ...
  try {
    ...
    await applyV21FinopsRls(client);
    await client.query('commit');
    console.log('✅ Migración V2.1 FinOps RLS aplicada (o ya estaba aplicada — es idempotente).');
  } catch (err) {
...
module.exports = { applyV21FinopsRls, SQL_PATH };
