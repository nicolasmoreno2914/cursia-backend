...
// rulesVersion 2 (5B.2.B + Fase 6) — spec
// 2026-09-25-cursia-v2-rules-v2-and-context-package-design §3 (DB). Aditiva,
// idempotente y SOLO staging: la cablea deploy-staging.yml (nunca deploy.yml),
// después de migrate-dynamic-generation.js (necesita generation_item_runs).
//
...
function assertExplicitStagingIntent() {
  if (process.env.MIGRATION_ENV !== 'staging') {
    console.error(
      ...
      '   staging, usá: MIGRATION_ENV=staging node scripts/migrate-dynamic-generation-v2.js'
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  try {
    const sql = fs.readFileSync(
      path.resolve(__dirname, '../supabase-migration-dynamic-generation-v2.sql'),
      'utf8',
    );
    await client.query('begin');
    await client.query(sql);
    await client.query('commit');
    console.log('✅ Migración dynamic-generation v2 (rulesVersion 2) aplicada (o ya estaba aplicada — es idempotente).');
  } catch (err) {
